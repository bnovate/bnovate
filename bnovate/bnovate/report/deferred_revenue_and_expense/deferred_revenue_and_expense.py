...
# TODO:
# - Adapt query to work with deferred expense
# - Remove Date Range filter, accept only Fiscal Year OR workaround "toggle_filter_display"
# - Fix chart
# - Fix simulation of the future.
...
class Deferred_Revenue_and_Expense_Report(object):
	...
	def get_invoices(self):
		...
		# )
		# TODO: integrate period_list filters.
		query = """
			SELECT
				inv.name AS doc,
				inv.posting_date,
				...
				inv_item.service_end_date,
				inv_item.base_net_amount,
				inv_item.enable_deferred_revenue,
				inv_item.deferred_revenue_account,
			...
			JOIN `tabGL Entry` gle ON inv_item.name = gle.voucher_detail_no AND inv_item.deferred_revenue_account = gle.account
			WHERE inv.docstatus = 1
				AND inv_item.enable_deferred_revenue = 1
			GROUP BY inv.name, inv_item.name, gle.posting_date
			ORDER BY gle.posting_date
		"""
		self.invoices = frappe.db.sql(query, as_dict=True)
